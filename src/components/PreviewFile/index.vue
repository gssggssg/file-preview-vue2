<template>
  <div class="container-preview-file">
    <div class="output" ref="output" v-if="!fileIdisNull">
      ...
      <Excel style="height: calc(100vh - 90px);" :excel="curFile.src"
        v-if="curFile.extend == 'xls' || curFile.extend == 'xlsx'"></Excel>
      <Word :docx="curFile.src" v-if="curFile.extend == 'docx'"></Word>
      <pptx :file="curFile.file" v-if="curFile.extend == 'ppt' || curFile.extend == 'pptx'"></pptx>
      <MyVideo style=" margin: 100px auto;width: 900px; height: 600px;" :video="curFile.video"
        v-if="videoTypes.includes(curFile.extend)"></MyVideo>
      <Music :musicList="curFile.musicList" v-if="musicTypes.includes(curFile.extend)"></Music>
      <MyImage :src="curFile.src" v-if="imageTypes.includes(curFile.extend)"></MyImage>
      <Codemirror v-if="textTypes.includes(curFile.extend)" style="top: 130px;" :initCodeValue="curFile.codeValue"
        :initEditorMode="curFile.cmMode"></Codemirror>
      <div v-if="curFile.extend == undefined" class="desc">
        <p> 不支持{{ extend }}格式的在线预览，请下载后预览或转换为支持的格式 </p>
        <p> {{ remindInfo }} </p>
      </div>
    </div>
...
export default {
  ...
  watch: {
    file: {
      ...
      deep: true,
      handler(value) {
        const { fileId, suffix, originalName } = value
        if (!fileId) {
          return
        }
        this.curFile = {
          extend: suffix,
          src: this.khConfig.host.rest + '/api/file/download/' + fileId,
        }
